...
from src.models import NNAlignEF_OLD, NNAlign
...
# With extra col
train_loader, train_dataset = get_NNAlign_dataloader(cedar_aligned.query('fold!=0 and fold != 1'), max_len=12,
                                                     window_size=5, seq_col='mutant', target_col='target',
                                                     feature_cols=feature_cols, sampler=RandomSampler,
                                                     return_dataset=True)
valid_loader, valid_dataset = get_NNAlign_dataloader(cedar_aligned.query('fold==0'), max_len=12, window_size=5,
                                                     seq_col='mutant', target_col='target', feature_cols=feature_cols,
                                                     sampler=SequentialSampler, return_dataset=True)
test_loader, test_dataset = get_NNAlign_dataloader(cedar_aligned.query('fold==1'), max_len=12, window_size=5,
                                                   seq_col='mutant', target_col='target', feature_cols=feature_cols,
                                                   sampler=SequentialSampler, return_dataset=True)

model = NNAlignEF_OLD(n_hidden, window_size, activation, batchnorm, dropout, indel, standardize,
