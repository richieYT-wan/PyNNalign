...
basecommand="python3 train_test_model_efsinglepass.py -trf ../data/NetMHCII_EL_jonas/HLA_DR_subsample_all_partitions.csv -tef ../data/NetMHCII_EL_jonas/HLA_DR_notsampled_200k_selected_test_set.csv -ml 21 -ws 9 -pad -20 -y target -x sequence -std False -bn False -nh 64 -br 10 -otf True -cuda True -bs 1024 -ne 500 -wd 0 -add_ps True -indel True --add_pfr True --add_fr_len True --add_pep_len True -lr 5e-5"


# Run 3 conditions :
# add nothing (BASELINE)
rid=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 5)
output_name="ALL_ON_HLA_DR_baseline"
for kf in {0..4};do
...
rid=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 5)
output_name="ALL_ON_HLA_DR_PerPositionStructure"
